...
public class Animal {
    ...
    public void move(MoveDirection dir){
        ...
        if(dir==MoveDirection.FORWARD||dir==MoveDirection.BACKWARD){
            positionChanged(tmp,this.getPosition());
        }
    ...
    private void positionChanged(Vector2d oldP, Vector2d newP){
        Iterator<IPositionChangeObserver> it=observers.iterator();
        while(it.hasNext()){
            it.next().positionChanged(oldP,newP);
        }
    }
